...
class OurMatrix:
    def __init__(self, nx, ny, frames):
        ...
        while t < frames:
            ...
            n = 0
            while n < nx:
                self.Data[n, :, t] = (np.exp(-((n - x_max)**2) / 50)) * impletion[n, :]  #модулируем заполнение по столбцам,
                n = n + 1
            k = 0    
            while k < ny:
                self.Data[:, k, t] = 10*(np.exp(-((k - y_max)**2) / 50)) * self.Data[:, k, t]  #а затем результат модулируем по строкам
                k = k + 1
